"""High-value service integration tests for analysis workflow.
...
from agent.api_schema import AnalyzeRequest, AnalyzeResponse
...
@pytest.fixture
def service(monkeypatch: pytest.MonkeyPatch) -> AnalysisService:
    import agent.rag

    # Prevent FAISS initialization during tests
    monkeypatch.setattr(
        agent.rag,
    ...
    monkeypatch.setattr(agent.rag, "preload_retriever_index", lambda *args, **kwargs: None)

    return AnalysisService(rag_index_dir="data/faiss_index")


def _assert_exact_decision_schema(response: AnalyzeResponse) -> Dict[str, object]:
    decision = response.decision.model_dump(by_alias=True)
...
def test_response_schema_integrity(
    ...
    """Call main analysis service and assert exact decision keys."""
    import agent.rag
    ...
    decision = _assert_exact_decision_schema(resp)
